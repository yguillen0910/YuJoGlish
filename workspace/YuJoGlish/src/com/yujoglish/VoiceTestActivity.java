/*
 * @(#) VoiceTestActivity.java  1 01/03/15
 * 
 ...
 */
public class VoiceTestActivity extends ARViewActivity implements RecognitionListener{
	 ...
	 */
	final static String POSICION_VOICE_TEST = "com.yujoglish.VoiceTestActivity";
	...
	private void inicializarVariables() {
		...
		infoRecibidaMenu = mensaje.getStringExtra(TestActivity.INFO_VOICE);
		infoRecibidaVoice = mensaje.getStringExtra(VoiceTestActivity.POSICION_VOICE_TEST);
		...
		String wordStr = WordUtils.capitalizeFully(palabraCorrecta.getNombre());
	 ...
	 */
	private void nextVoiceTest() 
	{
		Integer posicionSiguiente = Integer.parseInt(posicionPalabra)+1;
		...
		if(posicionSiguiente <= cantidadPalabras) {
			...
			myIntent.putExtra(POSICION_VOICE_TEST, infoEnvio);
			...
			myIntent.putExtra(POSICION_VOICE_TEST, "Has Finalizado el Test de Voz!");
			startActivity(myIntent);
		}
		this.finish();
	}
	 ...
	 * el mismo me enviará a la próxima pantalla Voice Test
	 * @param v vista actual
	 */
	public void onNextVoiceButtonClick(View v) {
		...
		guardarCantidadAciertosPalabraUsuario(0, "voice", idPalabraCorrecta);
		runActivityTime();	
	}
	...
	{
		guardarCantidadAciertosPalabraUsuario(1, "voice", idPalabraCorrecta);
		playSound("goodJob");
	}
	 ...
	 */
	public void guardarCantidadAciertosPalabraUsuario(Integer cantAciertos, String tipo, Integer idPalabraCorrecta){
		...
		verificarNivelUsuario(idUsuarioActivo);
	}
	
	/**
	 * Método para verificar si el usuario debe subir de nivel 
	 * @param idUsuarioActivo
	 */
	public void verificarNivelUsuario(String idUsuarioActivo){
		/*
		 * Cantidad de Registros en tabla Usuario_Palabra para usuario activo
		 */
		int cantidadRegistros = 0;
		
		/*
		 * Cantidad de aciertos minima para pasar de nivel 
		 */
		int cantidadAciertosMinima = 0;
		
		/*
		 * Nivel de Usuario Activo
		 */
		int nivelUsuarioActivo =0;
		/*
		 * Manejador de base de datos
		 */
		manager = new DataBaseManager(this);
		
		nivelUsuarioActivo = manager.obtenerNivelUsuarioActivo();
		if(nivelUsuarioActivo != 3){
			int cantidadPalabras = manager.obtenerCantPalabrasUsuarioActivo(nombreLeccion);
			cantidadRegistros = manager.obtenerCantidadRegistrosUsuarioPalabra(idUsuarioActivo);
			cantidadAciertosMinima = cantidadPalabras * 24; //24 es la cantidad de lecciones (12) * tipos de test (2)
			if(cantidadRegistros>=cantidadAciertosMinima){
				manager.modificarNivelUsuarioActivo();
			}
		}
	}
		...
		protected Void doInBackground(String... urls) {
	        ...
	        protected void onPostExecute(Void unused) {
	        	...
	        	if(Content == null){
	        		...
	        		System.out.println("Content no es null");
