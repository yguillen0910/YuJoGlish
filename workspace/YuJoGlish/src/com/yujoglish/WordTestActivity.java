/*
 * @(#) WordTestActivity.java  1 01/03/15
 * 
...
public class WordTestActivity extends ARViewActivity {
	 ...
	 */
	final static String POSICION_WORD = "com.yujoglish.WordTestActivity";
	/*
	...
	*/
	...
	private void cargarPalabrasButtons() 
	{
		...
		//se obntiene mensaje de la pantalla anterior
		Intent mensaje = getIntent();
		infoRecibidaMenu = mensaje.getStringExtra(TestActivity.INFO_WORD);
		infoRecibidaWord = mensaje.getStringExtra(WordTestActivity.POSICION_WORD); 
				
		if( infoRecibidaMenu != null){
			partes = infoRecibidaMenu.split("-");
		}else{
			partes = infoRecibidaWord.split("-");
		}
	...
	private void nextWordTest()
	{
		...
		if(posicionSiguiente<=cantidadPalabras){
			...
			myIntent.putExtra(POSICION_WORD, "Has Finalizado el Test de Palabra!");
			startActivity(myIntent);
		}
		this.finish();
	}
	...
	public void correcto () {
		guardarCantidadAciertosPalabraUsuario(1, "word", idPalabraCorrecta);
		playSound("goodJob");
	}
	...
	public void incorrecto() {
		...
		guardarCantidadAciertosPalabraUsuario(0, "word", idPalabraCorrecta);
		playSound("tryagain");
	}
